...
```
 Generate Mining Key
```
./target/release/impact generate-mining-key --chain=impact-archive
```
...
```
./target/release/impact import-mining-key "your_screet_seed" \--base-path /tmp/impactnode \--chain=impact-archive
```
 Collecting Data
```
./target/release/impact key insert \
  --base-path ~/.impactnode01 \
  --chain=impact-archive \
  --scheme Ed25519 \
