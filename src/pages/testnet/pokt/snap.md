...
<script>
  async function fetchSnapshotInfo() {
    try {
      const response = await fetch('https://snapshot.sychonix.com/testnet/pokt/log.json');
...
<div class="code-block-wrapper">
  <pre><code>sudo systemctl stop poktrolld</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>cp $HOME/.poktrolld/data/priv_validator_state.json $HOME/.poktrolld/priv_validator_state.json.backup</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>poktrolld tendermint unsafe-reset-all --home $HOME/.poktrolld --keep-addr-book</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>aria2c -x 16 -s 16 -o pokt-snapshot.tar.lz4 https://snapshot.sychonix.com/testnet/pokt/pokt-snapshot.tar.lz4
lz4 -dc pokt-snapshot.tar.lz4 | tar -xf - -C $HOME/.poktrolld
rm -v pokt-snapshot.tar.lz4</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>mv $HOME/.poktrolld/priv_validator_state.json.backup $HOME/.poktrolld/data/priv_validator_state.json</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>

<div class="code-block-wrapper">
  <pre><code>sudo systemctl restart poktrolld && sudo journalctl -u poktrolld -f -o cat</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
