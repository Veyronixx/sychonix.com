...
<script>
  async function fetchSnapshotInfo() {
    try {
      const response = await fetch('https://snapshot.sychonix.com/testnet/og/log.json');
...
<div class="code-block-wrapper">
  <pre><code>sudo systemctl stop 0gchaind</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>cp $HOME/.0gchain/data/priv_validator_state.json $HOME/.0gchain/priv_validator_state.json.backup</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>0gchaind tendermint unsafe-reset-all --home $HOME/.0gchain --keep-addr-book</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>aria2c -x 16 -s 16 -o og-snapshot.tar.lz4 https://snapshot.sychonix.com/testnet/og/og-snapshot.tar.lz4
lz4 -dc og-snapshot.tar.lz4 | tar -xf - -C $HOME/.0gchain
rm -v og-snapshot.tar.lz4</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>mv $HOME/.0gchain/priv_validator_state.json.backup $HOME/.0gchain/data/priv_validator_state.json</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>

<div class="code-block-wrapper">
  <pre><code>sudo systemctl restart 0gchaind && sudo journalctl -u 0gchaind -f -o cat</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
