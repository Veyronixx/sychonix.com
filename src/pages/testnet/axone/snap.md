...
<script>
  async function fetchSnapshotInfo() {
    try {
      const response = await fetch('https://snapshot.sychonix.com/testnet/axone/log.json');
...
<div class="code-block-wrapper">
  <pre><code>sudo systemctl stop axoned</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>cp $HOME/.axoned/data/priv_validator_state.json $HOME/.axoned/priv_validator_state.json.backup</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>axoned tendermint unsafe-reset-all --home $HOME/.axoned --keep-addr-book</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>aria2c -x 16 -s 16 -o axone-snapshot.tar.lz4 https://snapshot.sychonix.com/testnet/axone/axone-snapshot.tar.lz4
lz4 -dc axone-snapshot.tar.lz4 | tar -xf - -C $HOME/.axoned
rm -v axone-snapshot.tar.lz4</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>mv $HOME/.axoned/priv_validator_state.json.backup $HOME/.axoned/data/priv_validator_state.json</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>

<div class="code-block-wrapper">
  <pre><code>sudo systemctl restart axoned && sudo journalctl -u axoned -f -o cat</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
