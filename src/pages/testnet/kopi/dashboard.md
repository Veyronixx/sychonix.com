...
<div id="network-service" class="hidden">
  ...
  <div class="code-block-wrapper">
    <pre><code>c51fda38278d5d7985a08557c149fd0cc93dae96@kopi-testnet.sychonix.com:11656</code></pre>
    <button class="copy-btn"><i class="fas fa-copy"></i></button>
  </div>

  <label class="block mt-1 mb-1">Live Peers</label>
  <div class="code-block-wrapper">
    <pre><code>PEERS="$(curl -sS https://rpc-kopi-t.sychonix.com/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"  
sed -i -e "s|^persistent_peers *=.*|persistent_peers = \"$PEERS\"|" $HOME/  /config/config.toml</code></pre>
  ...
  <div class="code-block-wrapper">
    <pre><code>curl -Ls https://snapshot.sychonix.com/testnet/kopi/addrbook.json > $HOME/.kopid/config/addrbook.json</code></pre>
    <button class="copy-btn"><i class="fas fa-copy"></i></button>
  </div>
</div>
...
<div id="chain-explorer" class="hidden bg-slate-950 p-4 rounded-lg shadow-md">
  ...
  <p class="text-sm text-gray-300">
    <a href="https://explorer.sychonix.com/kopi-testnet" target="_blank" 
       class="text-blue-500 underline hover:text-blue-700 hover:shadow-lg">
      https://explorer.sychonix.com/kopi-testnet
    </a>
  </p>
</div>
