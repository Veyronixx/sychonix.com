...
<script>
  async function fetchSnapshotInfo() {
    try {
      const response = await fetch('https://snapshot.sychonix.com/testnet/selfchain/log.json');
...
<div class="code-block-wrapper">
  <pre><code>aria2c -x 16 -s 16 -o selfchain-snapshot.tar.lz4 https://snapshot.sychonix.com/testnet/selfchain/selfchain-snapshot.tar.lz4
