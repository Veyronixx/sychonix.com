...
- Autoinstall Script
```
source <(curl -s https://snapshot.service.sychonix.com/empeiria/autoinstall.sh) 
```
----
...
- Genesis
```
curl -Ls https://snapshot.service.sychonix.com/empeiria/genesis.json > $HOME/.empe-chain/config/genesis.json 
```
- Download addrbook
```
curl -Ls https://snapshot.service.sychonix.com/empeiria/addrbook.json > $HOME/.empe-chain/config/addrbook.json
```
- Configure Peers
```
PEERS="$(curl -sS https://rpc-empeiria-t.service.sychonix.com/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"
