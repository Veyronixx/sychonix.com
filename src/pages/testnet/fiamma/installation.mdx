...
- Autoinstall Script
```
source <(curl -s https://snapshot.service.sychonix.com/fiamma/autoinstall.sh) 
```
------
Manual installation
...
- Seed and Peer
```
PEERS="$(curl -sS https://rpc-fiamma-t.service.sychonix.com/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"
...
- Download Snapshot
```
curl "https://snapshot.service.sychonix.com/fiamma/fiamma-latest.tar.lz4" | lz4 -dc - | tar -xf - -C "$HOME/.fiamma"
```
