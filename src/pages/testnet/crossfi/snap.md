...
<script>
  async function fetchSnapshotInfo() {
    try {
      const response = await fetch('https://snapshot-2.sychonix.com/testnet/crossfi/log.json');
...
<div class="code-block-wrapper">
  <pre><code>cp $HOME/.crossfid/data/priv_validator_state.json $HOME/.crossfid/priv_validator_state.json.backup</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>aria2c -x 16 -s 16 -o crossfi-snapshot.tar.lz4 https://snapshot-2.sychonix.com/testnet/crossfi/crossfi-snapshot.tar.lz4
lz4 -dc crossfi-snapshot.tar.lz4 | tar -xf - -C $HOME/.crossfid
rm -v crossfi-snapshot.tar.lz4</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
