...
<script>
  async function fetchSnapshotInfo() {
    try {
      const response = await fetch('https://snapshot.sychonix.com/mainnet/aaronetwork/log.json');
...
<div class="code-block-wrapper">
  <pre><code>sudo systemctl stop aaronetworkd</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>cp $HOME/.aaronetwork/data/priv_validator_state.json $HOME/.aaronetwork/priv_validator_state.json.backup</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>aaronetworkd tendermint unsafe-reset-all --home $HOME/.aaronetwork --keep-addr-book</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>aria2c -x 16 -s 16 -o aaronetwork-snapshot.tar.lz4 https://snapshot.sychonix.com/mainnet/aaronetwork/aaronetwork-snapshot.tar.lz4
lz4 -dc aaronetwork-snapshot.tar.lz4 | tar -xf - -C $HOME/.aaronetwork
rm -v aaronetwork-snapshot.tar.lz4</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
...
<div class="code-block-wrapper">
  <pre><code>mv $HOME/.aaronetwork/priv_validator_state.json.backup $HOME/.aaronetwork/data/priv_validator_state.json</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>

<div class="code-block-wrapper">
  <pre><code>sudo systemctl restart aaronetworkd && sudo journalctl -u aaronetworkd -f -o cat</code></pre>
  <button class="copy-btn"><i class="fas fa-copy"></i></button>
</div>
