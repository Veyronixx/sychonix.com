...
<script>
  async function fetchSnapshotInfo() {
    try {
      const response = await fetch('https://snapshot.sychonix.com/mainnet/symphony/log.json');
...
<div class="code-block-wrapper">
  <pre><code>aria2c -x 16 -s 16 -o symphony-snapshot.tar.lz4 https://snapshot.sychonix.com/mainnet/symphony/symphony-snapshot.tar.lz4
