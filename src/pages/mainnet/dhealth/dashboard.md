...
<div class="dashboard-overview p-6 bg-gray-900 rounded-lg mb-2"> <!-- Changed mb-6 to mb-2 -->
  <div class="flex flex-col items-center">
    ...
    <img src="/img/dhealth.jpg" alt="Logo" class="dashboard-logo mb-2 w-24 h-24 rounded-full" />
...
<div id="public-endpoints">
  ...
  <div class="code-block-wrapper">
    <pre><code>https://rpc-dhealth.sychonix.com</code></pre>
    <button class="copy-btn"><i class="fas fa-copy"></i></button>
  </div>

  <label>API Endpoints</label>
  <div class="code-block-wrapper">
    <pre><code>https://api-dhealth.sychonix.com</code></pre>
    <button class="copy-btn"><i class="fas fa-copy"></i></button>
  </div>
</div>
...
<div id="network-service" class="hidden">
  ...
  <div class="code-block-wrapper">
    <pre><code>PEERS="$(curl -sS https://rpc-dhealth.sychonix.com/net_info | jq -r '.result.peers[] | "\(.node_info.id)@\(.remote_ip):\(.node_info.listen_addr)"' | awk -F ':' '{print $1":"$(NF)}' | sed -z 's|\n|,|g;s|.$||')"  
sed -i -e "s|^persistent_peers *=.*|persistent_peers = \"$PEERS\"|" $HOME/.dhealth/config/config.toml</code></pre>
    <button class="copy-btn"><i class="fas fa-copy"></i></button>
  </div>
  ...
  <div class="code-block-wrapper">
    <pre><code>curl -Ls https://snapshot.sychonix.com/mainnet/dhealth/addrbook.json > $HOME/.dhealth/config/addrbook.json</code></pre>
    <button class="copy-btn"><i class="fas fa-copy"></i></button>
  </div>
</div>
...
<div id="chain-explorer" class="hidden bg-slate-950 p-4 rounded-lg shadow-md">
  ...
  <p class="text-sm text-gray-300">
    <a href="https://explorer.sychonix.com/dhealth-mainnet" target="_blank" 
       class="text-blue-500 underline hover:text-blue-700 hover:shadow-lg">
      https://explorer.sychonix.com/dhealth-mainnet
    </a>
  </p>
</div>
